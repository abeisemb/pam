...
class TestPamBz(object):
    def test_2068461(self, multihost, create_localusers, bkp_pam_config):
        ...
        command = client.run_command(
            f"echo x86_64_baseos_rpms | passwd --stdin local_anuj", raiseonerr=False).stderr_text
        ...
        command = multihost.client[0].run_command(
            f"echo x86#64#baseos#rpms | passwd --stdin local_anuj", raiseonerr=False).stderr_text
        assert "failed" in command or "exhausted" in command
        ...
        for passwd in ['HI_I_AM_ANUJ',
            ...
            execute_cmd(multihost, f"echo {passwd} | passwd --stdin local_anuj")
        command = client.run_command(
            f"echo HI_I_AM_ANUJ | passwd --stdin local_anuj", raiseonerr=False).stderr_text
        assert "failed" in command or "exhausted" in command
